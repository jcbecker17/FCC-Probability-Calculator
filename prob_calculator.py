    ...
    def draw(self, draw_num):
        if draw_num > len(self.contents):       # If the number of balls to draw exceeds the available quantity, return all the balls.
            ...
            for _ in range(draw_num):
                ...
                index = int(round((contents_length-1) * random.random()))   # Choose a random index from 0 to the number of balls remaining in the hat
...
def experiment(hat, expected_balls, num_balls_drawn, num_experiments):
    n = 0   # number of experiments completed so far
    ...
    for _ in range(num_experiments):            # This loop represents a single experiment. It executes the number of times specified by the num_experiments argument
        ...
        hat_copy.draw(num_balls_drawn)          # Draw random balls from the hat contents, and return the selection list
        ...
        for elem in hat_copy.selection:         # Iterate over elements in the selection list
        ...
        n += 1          # An experiment has been completed, regardless of success, so increment n

    prob = m/n          # Calculate and return the probability of success
    return prob
